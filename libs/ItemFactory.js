...
exports.Factory.prototype.sitemapUrl = function() {
  ...
  if (this.platform.username && this.platform.password) {
    serverString =
      encodeURIComponent(this.platform.username) +
      ":" +
...
exports.Factory.prototype.parseSitemap = function(jsonSitemap) {
  ...
  for (var key in this.itemList) {
    if (this.itemList.hasOwnProperty(key)) {
      ...
      this.itemList[key] = exports.Factory.prototype.checkCustomAttrs(
        this,
        key,
        this.platform,
        this.catList
      ...
      if (!(this.itemList[key].type in exports)) {
        this.log(
          "Platform - The widget '" +
            this.itemList[key].name +
            "' of type " +
            this.itemList[key].type +
            " is an item not handled."
        );
        continue;
      }
      ...
      var accessory = new exports[this.itemList[key].type](
        this.itemList[key],
        this.platform,
        this.homebridge
      );
      ...
      if (accessoryList.length > 99) {
        ...
        this.log(
          "Platform - Accessory count limit (100) exceeded so skipping: '" +
            this.itemList[key].name +
            "' of type " +
            this.itemList[key].type +
            " was skipped."
        ...
        if (this.platform.rooms.length == 0) {
          ...
          if (control.room) {
            ...
            if (this.platform.rooms.indexOf(controlRoom) >= 0) {
              if (
                this.platform.moodSwitches == "only" &&
                this.itemList[key].type !== "LightControllerV2MoodSwitch"
              ) {
              ...
              this.log(
                "Platform - Skipping as room " +
                  controlRoom +
                  " is not in the config.json rooms list."
              );
            }
          } else {
            // cannot add this accessory as it does not have a room
            this.log(
              "Platform - Skipping as could not determine which room the accessory is in."
            );
          }
        }
      }
  ...
  this.log(
    "Platform - Total accessory count " +
      accessoryList.length +
      " across " +
      this.platform.rooms.length +
      " rooms."
  );
  return accessoryList;
};

exports.Factory.prototype.checkCustomAttrs = function(
  factory,
  itemId,
  platform,
  catList
  ...
  if (item.name.startsWith("Temperat")) {
    ...
    catList[item.cat] !== undefined &&
    catList[item.cat].image === "00000000-0000-0002-2000000000000000.svg"
  ) {
    ...
    item.parentType === "LightController" ||
    item.parentType === "LightControllerV2"
  ) {
  ...
  if (item.type == "InfoOnlyDigital") {
    if (item.defaultIcon == "00000000-0000-0021-2000000000000000") {
      ...
      item.name.indexOf("Motion") !== -1 ||
      item.name.indexOf("Presence") !== -1
    ) {
  ...
  if (item.type == "InfoOnlyAnalog") {
    if (item.name.indexOf("Door Contact") !== -1) {
      ...
      (item.name.indexOf("Motion") !== -1 ||
        item.name.indexOf("Presence") !== -1) &&
      item.name.indexOf("Brightness") == -1
    ) {
      item.type = "MotionSensor";
    } else if (
      item.name.indexOf("Brightness") !== -1 ||
      item.name.indexOf("Light Level") !== -1
    ) {
  ...
  if (
    item.uuidAction.indexOf("/masterValue") !== -1 ||
    item.uuidAction.indexOf("/masterColor") !== -1
  ) {
        ...
        for (var controlUuid in controls) {
          if (controls.hasOwnProperty(controlUuid)) {
            ...
            if (
              ...
              (factory.platform.moodSwitches == "all" ||
                factory.platform.moodSwitches == "only")
            ) {
              var moods = JSON.parse(
                factory.platform.ws.getLastCachedValue(control.states.moodList)
              );
              ...
              for (var r = 0; r < moods.length; r++) {
                ...
                moodSwitchControl.name =
                  "Mood " + mood.name + " of " + control.name;
