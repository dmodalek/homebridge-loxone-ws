...
module.exports = function(homebridge) {
  ...
  Utility.addSupportTo(
    ItemFactory.LightControllerV2MoodSwitch,
...
function LoxPlatform(log, config) {
  ...
  if (!this.config["host"])
    throw new Error(
      "Configuration missing: loxone host (please provide the IP address here)"
    );
  if (!this.config["port"])
    throw new Error(
      "Configuration missing: loxone port (if default port, specify 7777)"
    );
  if (!this.config["username"])
    throw new Error("Configuration missing: loxone username");
  if (!this.config["password"])
    throw new Error("Configuration missing: loxone password");
  if (!this.config["rooms"])
    this.log("Info: rooms array not configured. Adding every room.");
...
LoxPlatform.prototype.accessories = function(callback) {
  ...
  this.log(
    "Platform - Waiting 8 seconds until initial state is retrieved via WebSocket."
  );
  setTimeout(function() {
